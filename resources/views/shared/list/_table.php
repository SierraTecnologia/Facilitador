...
<table class="table listing columns-<?=count($columns)?>">
			...
			<tr>
				...
				<?php if (isset($test_actions)): ?>
					<?php if (count($test_actions)): ?>
						<th class="actions-<?=count($test_actions)?>"><?= __('facilitador::list.table.actions') ?></th>
					<?php endif ?>
				<?php else: ?>
					<th class="actions-3"><?= __('facilitador::list.table.actions') ?></th>
				<?php endif ?>
	...
	<tbody>
			...
			<tr class="hide warning bulk-actions">
				<td colspan="999">
					<a class="btn btn-warning remove-confirm" href="#">
						...
						<?= __('facilitador::list.table.remove_selected') ?>
					</a>
				</td>
			</tr>
		...
		<?php else: ?>
			<?=View::make('facilitador::shared.list._bulk_actions')->render()?>
		<?php endif ?>
		...
		<?=View::make('facilitador::shared.list._no_results', $__data)->render()?>
