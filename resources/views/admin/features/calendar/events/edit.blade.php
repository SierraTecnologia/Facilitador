        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="btn-toolbar float-right mt-2 mb-4">
                    @if (! siravel()->isDefaultLanguage() && $event->translationData(request('lang')))
                        ...
                        @endif
                        <a class="btn btn-warning ml-1" href="{!! Siravel::rollbackUrl($event->translation(request('lang'))) !!}">Rollback</a>
                    @else
                        ...
                        @endif
                        <a class="btn btn-warning ml-1" href="{!! Siravel::rollbackUrl($event) !!}">Rollback</a>
        ...
        <div class="row">
            <div class="@if (\Illuminate\Support\Facades\Config::get('siravel.live-preview', false)) col-md-6 @else col-md-12 @endif">
                {!! Form::model($event, ['route' => ['admin.events.update', $event->id], 'method' => 'patch', 'class' => 'edit']) !!}
                    ...
                    {!! FormMaker::setColumns(3)->fromObject($event->asObject(), \Illuminate\Support\Facades\Config::get('siravel.forms.event.identity')) !!}
                    {!! FormMaker::setColumns(1)->fromObject($event->asObject(), \Illuminate\Support\Facades\Config::get('siravel.forms.event.content')) !!}
                    {!! FormMaker::setColumns(2)->fromObject($event->asObject(), \Illuminate\Support\Facades\Config::get('siravel.forms.event.seo')) !!}
                    {!! FormMaker::setColumns(2)->fromObject($event->asObject(), \Illuminate\Support\Facades\Config::get('siravel.forms.event.publish')) !!}
