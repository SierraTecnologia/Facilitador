...
@section('content')
    ...
    <div class="row">
        @if (! is_null(request('lang')) && request('lang') !== \Illuminate\Support\Facades\Config::get('cms.default-language', 'en') && $event->translationData(request('lang')))
    ...
    <div class="row raw-margin-bottom-24">
        <ul class="nav nav-tabs">
            @foreach(\Illuminate\Support\Facades\Config::get('cms.languages', Cms::\Illuminate\Support\Facades\Config::get('cms.languages')) as $short => $language)
                <li role="presentation" @if (request('lang') == $short || is_null(request('lang')) && $short == Cms::\Illuminate\Support\Facades\Config::get('cms.default-language'))) class="active" @endif><a href="{{ url('admin/events/'.$event->id.'/edit?lang='.$short) }}">{{ ucfirst($language) }}</a></li>
            @endforeach
        </ul>
    </div>
