        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered">
                    ...
                    <form role="form"
                            class="form-edit-add"
                            action="{{ $edit ? \Facilitador\Routing\UrlGenerator::managerRoute($dataType->slug, 'update', $dataTypeContent->getKey()) : \Facilitador\Routing\UrlGenerator::managerRoute($dataType->slug, 'store') }}"
                        ...
                        <div class="panel-body">
                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                    ...
                                    @include('facilitador::multilingual.input-hidden-bread-edit-add')
                                    @if (isset($row->details->view))
                                        @include($row->details->view, ['row' => $row, 'dataType' => $dataType, 'dataTypeContent' => $dataTypeContent, 'content' => $dataTypeContent->{$row->field}, 'action' => ($edit ? 'edit' : 'add'), 'view' => ($edit ? 'edit' : 'add'), 'options' => $row->details])
                                    @elseif ($row->type == 'relationship')
                                        @include('facilitador::components.formfields.relationship', ['options' => $row->details])
                                    @else
                    ...
                    <form id="my_form" action="{{ route('facilitador.upload') }}" target="form_target" method="post"
