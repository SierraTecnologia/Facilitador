...
Facilitador comes with Roles and Permissions out of the box. Each _User_ has a _Role_ which has a set of _Permissions_.
...
New in version 1.0, we've changed Facilitador's authorization system to be [more in line with Laravel](https://laravel.com/docs/authorization#authorizing-actions-using-policies)! This means that you can check for permissions in the following ways:
...
* `browse_admin`: Whether or not the user may browse the Facilitador admin panel.
* `browse_database`: Whether or not the user may browse the Facilitador database menu section.
* `browse_bread`: Whether or not the user may browse the Facilitador BREAD menu section.
* `browse_media`: Whether or not the user may browse the Facilitador media section.
* `browse_menu`: Whether or not the user may browse the Facilitador menu section.
* `browse_settings`: Whether or not the user may browse the Facilitador settings section.
...
```php
  ...
  {
      return Auth::guard(app('FacilitadorGuard'));
  }
```
