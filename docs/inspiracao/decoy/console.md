...
If you make a Laravel command extend from `Facilitador\Models\Worker`, the command is embued with some extra functionality.  The following options get added:
...
In addition, by subclassing `Facilitador\Models\Worker`, the worker command will show up in a listing in the admin at /admin/workers.  From this interface you can make sure the worker is still running and view logs.
