...
Then, models that support encoding should use the `Support\Traits\Models\Encodable` trait.  You also need to itemize each encodable attribute on the model by defining a `$encodable_attributes` property on the model.
