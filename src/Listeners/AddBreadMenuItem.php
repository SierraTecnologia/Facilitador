...
class AddBreadMenuItem
{
    ...
    {
        if (config('facilitador.bread.add_menu_item') && file_exists(base_path('routes/web.php'))) {
            $menu = Facilitador::model('Menu')->where('name', config('facilitador.bread.default_menu'))->firstOrFail();
