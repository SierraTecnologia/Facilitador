...
class FacilitadorBreadController extends Controller
{
    ...
    public function create(Request $request, $table)
    {
        ...
        return Facilitador::view('facilitador::tools.bread.edit-add', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data = $res
                ? $this->alertSuccess(__('facilitador::cruds.bread.success_created_bread'))
                : $this->alertError(__('facilitador::cruds.bread.error_creating_bread'));
            if ($res) {
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = Facilitador::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return Facilitador::view('facilitador::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \Support\Models\Application\DataType $dataType */
        try {
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('facilitador::cruds.bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('facilitador::cruds.bread.error_updating_bread'));
            if ($res) {
    ...
    public function destroy($id)
    {
        ...
        $data = $res
            ? $this->alertSuccess(__('facilitador::cruds.bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('facilitador::cruds.bread.error_updating_bread'));
        if ($res) {
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = Facilitador::modelClass('DataRow');
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = Facilitador::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = Facilitador::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
