...
use TCG\Facilitador\Events\FileDeleted;
use TCG\Facilitador\Http\Controllers\ContentTypes\Checkbox;
use TCG\Facilitador\Http\Controllers\ContentTypes\Coordinates;
use TCG\Facilitador\Http\Controllers\ContentTypes\File;
use TCG\Facilitador\Http\Controllers\ContentTypes\Image as ContentImage;
use TCG\Facilitador\Http\Controllers\ContentTypes\MultipleCheckbox;
use TCG\Facilitador\Http\Controllers\ContentTypes\MultipleImage;
use TCG\Facilitador\Http\Controllers\ContentTypes\Password;
use TCG\Facilitador\Http\Controllers\ContentTypes\Relationship;
use TCG\Facilitador\Http\Controllers\ContentTypes\SelectMultiple;
use TCG\Facilitador\Http\Controllers\ContentTypes\Text;
use TCG\Facilitador\Http\Controllers\ContentTypes\Timestamp;
use TCG\Facilitador\Traits\AlertsMessages;
use Validator;
...
class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            $data->save();
            if ($old_path != $new_path && !Storage::disk(config('facilitador.storage.disk'))->exists($new_path)) {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('facilitador.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('facilitador.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if (Storage::disk(config('facilitador.storage.disk'))->exists($path)) {
            Storage::disk(config('facilitador.storage.disk'))->delete($path);
